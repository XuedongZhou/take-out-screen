<template>
    <div class="base-scroll-list" :id="id">
        ...
        >
            <div
                class="header-item ellipsis-text"
                v-for="(headerItem, i) in headerData"
                :key="headerItem + i"
                :style="{
                    color: actualConfig?.headerColor,
                    fontSize: actualConfig?.headerFontSize,
        ...
        <div class="list-rows-wrapper" :style="{ height: rowsHeight }">
            <div
                class="list-rows"
                v-for="(row, index) in currentRowsData"
                :key="row.rowIndex"
                :style="{
                    height: rowsHeights[index],
                    lineHeight: rowsHeights[index],
                    backgroundColor:
                        row.rowIndex % 2 === 0 ? actualConfig?.rowBackground[0] : actualConfig?.rowBackground[1]
                }"
            >
                <div
                    class="list-columns ellipsis-text"
                    v-for="(col, colIndex) in row.data"
                    :key="colIndex"
                    :style="{
                        color: actualConfig?.rowColor,
                        fontSize: actualConfig?.rowFontSize,
...
import { watch, ref, type CSSProperties, computed, onMounted } from 'vue';
...
async function handleHeader(config: Config) {
    ...
    const cloneAligns = new Array(cloneHeaderData.length)
        .fill('center')
        .map((item, index) => (config.aligns[index] ? config.aligns[index] : item));
    ...
    if (config.headerIndex) {
        ...
        cloneData.forEach((rows, index) => {
            if (
                ...
                config.headerIndexData[index] !== undefined
            ) {
